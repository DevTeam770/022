...
export function TechnicianDashboardPage() {
  ...
  const handleApplyNameToCucm = async () => {
    ...
    try {
      ...
      toast.success("השם עודכן ב-CUCM בהצלחה — ניתן לאשר את הבקשה");
    } catch (err) {
      setCucmApplyStatus("error");
      toast.error(err.message || "שגיאה בעדכון השם ב-CUCM — יש לדחות את הבקשה");
    } finally {
      setCucmApplyLoading(false);
    }
  };
            ...
            <div className="space-y-4 text-right">
              ...
              {selectedRequest.type === "name-change" && (
                <div className="space-y-2">
                  ...
                  {cucmApplyStatus === "success" && (
                    <p className="text-sm font-medium text-emerald-600">
                      ✓ העדכון בוצע בהצלחה ב-CUCM — ניתן לאשר את הבקשה
                    </p>
                  )}
                  {cucmApplyStatus === "error" && (
